...
IDXGISwapChain* GetSwapChain()
{
	...
	std::string WindowName = es("KKALUMFFERS");
...
int AimBot_API(float X, float Y) // 에임봇 마우스이벤트 로직
{
	...
	if (abs((LONG)X * cfg->AimSmooth) > 1)
		InputMouse.mi.dx = (LONG)X * (8 * (cfg->AimSmooth / 50));
	if (abs((LONG)Y * cfg->AimSmooth) > 1)
		InputMouse.mi.dy = (LONG)Y * (8 * (cfg->AimSmooth / 50));
		...
		if (cfg->Aimbot) {
			if (GetAsyncKeyState(VK_RBUTTON) && (int)UGameData->GetCurrentWeaponIndex() <= 3 && (int)UGameData->GetCurrentWeaponIndex() >= 0) {
				...
				if (a.pMesh != 0x0 && a.Actor != 0x0) {
					...
					if (GetVisBone(GlobalCamera.POV.Location, returnvec3)) {
...
long __stdcall hk_present(IDXGISwapChain* p_swapchain, unsigned int syncintreval, unsigned int flags)
{
	...
	if(!islogin) {
		...
		if (ImGui::Button(es("Login"), ImVec2(95, 25))) {
			std::string Hash = "";
			srand(time(0));
			for (int i = 0; i < 32; ++i) {
				Hash += genRandom();
		...
		if (g_ShowMenu) {
			...
			ImGui::Begin(es("LAST - PUBG"), &g_ShowMenu);
			ImGui::Checkbox(es("Skeleton ESP"), &cfg->Skelcfg);
			ImGui::SameLine();
			ImGui::Checkbox(es("Player Info"), &cfg->Playercfg);
			ImGui::Checkbox(es("No Recoil / Aimbot"), &cfg->Aimbot);
			ImGui::SliderFloat("AimSmooth", &cfg->AimSmooth, 0.00000f, 5.0f, "%0.2f");
			//ImGui::SliderFloat("AimFov", &cfg->Fov, 0.00000f, 500.0f, "%.1f");
			ImGui::Checkbox(es("Radar"), &cfg->Radarcfg);
			ImGui::SameLine();
			//ImGui::Checkbox(es("Item ESP"), &cfg->Itemcfg);
...
void HookD3D11()
{
	...
	if (m_pSwapChain)
	{
		...
		DetourAttach(&(LPVOID&)o_present, hk_present);
		DetourAttach(&(LPVOID&)o_BitBlt, Hooked_BitBlt);
